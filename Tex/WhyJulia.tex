...
performance than other scripting languages by how it uses type
inference.
...
\texttt{Vector\{Any\}}. For example:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}14}]:} \PY{n}{a} \PY{o}{=} \PY{k+kt}{Vector}\PY{p}{\PYZob{}}\PY{k+kt}{Any}\PY{p}{\PYZcb{}}\PY{p}{(}\PY{n}{undef}\PY{p}{,}\PY{l+m+mi}{3}\PY{p}{)}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}14}]:} 3-element Array\{Any,1\}:
          1.0       
           "hi!"    
           :Symbolic
\end{Verbatim}
...
what it sounds like. For example:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}15}]:} \PY{n}{a} \PY{o}{=} \PY{k+kt}{Vector}\PY{p}{\PYZob{}}\PY{k+kt}{Union}\PY{p}{\PYZob{}}\PY{k+kt}{Float64}\PY{p}{,}\PY{k+kt}{Int}\PY{p}{\PYZcb{}}\PY{p}{\PYZcb{}}\PY{p}{(}\PY{n}{undef}\PY{p}{,}\PY{l+m+mi}{3}\PY{p}{)}
         \PY{n}{a}\PY{p}{[}\PY{l+m+mi}{1}\PY{p}{]} \PY{o}{=} \PY{l+m+mf}{1.0}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}15}]:} 3-element Array\{Union\{Float64, Int64\},1\}:
          1.0 
          3   
          0.25
\end{Verbatim}
...
function:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}16}]:} \PY{n+nd}{@code\PYZus{}warntype} \PY{l+m+mi}{2}\PY{o}{\PYZca{}}\PY{l+m+mi}{5}
\end{Verbatim}
...
typed. What about in our \texttt{expo}?
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}17}]:} \PY{n+nd}{@code\PYZus{}warntype} \PY{n}{expo}\PY{p}{(}\PY{l+m+mi}{2}\PY{p}{,}\PY{l+m+mi}{5}\PY{p}{)}
\end{Verbatim}
...
\textcolor{ansi-black-intense}{│  }\textcolor{ansi-black-intense}{  }└──       return \%10
    ...
    Notice that possible returns are the temporary \texttt{\%4} and
\texttt{\%10} which are different types, and so the return type is
inferred as \texttt{Union\{Float64,Int64\}}. To trace exactly to where
this instability occurs, we can use Traceur.jl:

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}19}]:} \PY{k}{using} \PY{n}{Traceur}
         \PY{n+nd}{@trace} \PY{n}{expo}\PY{p}{(}\PY{l+m+mi}{2}\PY{p}{,}\PY{l+m+mi}{5}\PY{p}{)}
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}19}]:} 32
\end{Verbatim}
            
    This tells us that on line 2 \texttt{x} is assigned to an \texttt{Int}
while on line 5 it's assigned to a \texttt{Float64}, and so it's
inferred as \texttt{Union\{Float64,\ Int64\}}. Line 5 is where we put
the explicit \texttt{convert} call, so this identified exactly the issue
for us.
...
function like:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}17}]:} \PY{k}{function} \PY{n}{foo}\PY{p}{(}\PY{n}{array}\PY{p}{)}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}17}]:} foo (generic function with 1 method)
\end{Verbatim}
...
on elements:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}18}]:} \PY{k}{function} \PY{n}{inner\PYZus{}foo}\PY{p}{(}\PY{n}{val}\PY{p}{)}
           \PY{c}{\PYZsh{} Do algorithm X on val}
         \PY{k}{end}
\end{Verbatim}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}18}]:} inner\_foo (generic function with 1 method)
\end{Verbatim}
            
    and instead define foo as:

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}20}]:} \PY{k}{function} \PY{n}{foo2}\PY{p}{(}\PY{n}{array}\PY{o}{::}\PY{k+kt}{Array}\PY{p}{)}
           \PY{k}{for} \PY{n}{i} \PY{k+kp}{in} \PY{n}{eachindex}\PY{p}{(}\PY{n}{array}\PY{p}{)}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}20}]:} foo2 (generic function with 1 method)
\end{Verbatim}
...
function.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}21}]:} \PY{k}{function} \PY{n}{test}\PY{p}{(}\PY{n}{x}\PY{p}{)}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}21}]:} test (generic function with 1 method)
\end{Verbatim}
...
highest scope (and thus effectively the global scope):
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}22}]:} \PY{n}{a} \PY{o}{=} \PY{l+m+mi}{3}
...
to specify variables as constant:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}23}]:} \PY{k+kd}{const} \PY{n}{a\PYZus{}cons} \PY{o}{=} \PY{l+m+mi}{3}
         \PY{k}{function} \PY{n}{badidea}\PY{p}{(}\PY{p}{)}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}23}]:} badidea (generic function with 1 method)
\end{Verbatim}
            
    Beware that functions will specialize using the value of the constants,
so they should go unchanged after being set.
...
error to see is for newcomers to benchmark Julia like:
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}25}]:} \PY{n}{a} \PY{o}{=} \PY{l+m+mf}{3.0}
...
will optimize away the loop and stick in the answer)
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}26}]:} \PY{k}{function} \PY{n}{timetest}\PY{p}{(}\PY{p}{)}
             \PY{n}{a} \PY{o}{=} \PY{l+m+mf}{3.0}
             \PY{n+nd}{@time} \PY{k}{for} \PY{n}{i} \PY{o}{=} \PY{l+m+mi}{1}\PY{o}{:}\PY{l+m+mi}{4}
                 \PY{n}{a} \PY{o}{+=} \PY{n}{i}
             \PY{k}{end}
